...
def sentence1() -> Expr:
    ...
    # First we construct the symbols 
    A = Expr('A')
    B = Expr('B')
    C = Expr('C')

    # Now we construct the expressions

    # We can skip the disjoin because we have only two elements
    first = A | B

    # We can skip the disjoin because we have only two elemenets
    second = (~A) % (~B | C)

    # Now we must use the disjoin otherwise it will create a nested logic tree
    third = disjoin((~A), (~B), C)

    # We must also use conjoin firstly because the pronuncation asked us to 
    # and because if we don't it will create a nested logic tree
...
def sentence2() -> Expr:
    ...
    # We can skip the conjoin because we have only two elements
    second = A >> ((~B) & (~D))

    # We can skip the conjoin because we have only two elements
    third = (~(B & (~C))) >> A

    # A simple implies statement
    fourth = (~D) >> C
...
def sentence3() -> Expr:
    ...
    # We need to use conjoin and disjoin in order to avoid nested logic tree since we have more than two elements
    first = PacmanAlive_1 % disjoin(conjoin(PacmanAlive_0, ~PacmanKilled_0), conjoin(~PacmanAlive_0, PacmanBorn_0))

    # Both alive and be born can be expressed without conjoin because we have only two elements
    # and we will add a not to show the «cannot»
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # We are making the representation of the lower cased expression Expr('a') and return a model where it is True based on the above
    # The one-line return statement, not much to say in this one
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # We are using the defintion of the entailment and not satisfaction
    # We are constructing the φ and not ψ expressions 
    # In order for the statement to be true it needs the expression φ and not ψ to be unsatisfied
    # In otherwords the expression doesn't have a model that would make it true
    # As shown above in this situation the findModel will return False showing that there are no models
    # Otherwise if it is satisfied then we return False because there will be at least one model
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # We are passing as argument the not inverse_statement and the models as we were told above
    # The pl_true will return True if statement is true given the model or False if it is false
    # If the assignments does not containt a model for the given statement it will return None
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # The disjoin function can take as argument a list of literals and return their disjunction
    # Which is what we want to do since we are looking for atLeastOne True expression
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # Store every disjoined combination in a list
    clauses = [disjoin(~item[0], ~item[1]) for item in itertools.combinations(literals, 2)]

    # In the end conjoin all of them because we are looking for atMostOne to be True
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # exactlyOne can be expressed by atLeastOne and atMostOne
    # So we are returning exactly what i said above the conjoin of atLeastOne with the atMostOne
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    # for all (x, y) in all_coords:
    # If a wall is at (x, y) --> Pacman is not at (x, y)
    # Constructing a temporary list where we store the expression for walls and pacman
    temp_list = [PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t)) for (x, y) in all_coords]

    # Then conjoin them and append them to the final list.
    pacphysics_sentences.append(conjoin(temp_list))
    # Pacman is at exactly one of the squares at timestep t.
    # We are constructing a temporary list where we will append all the state (x,y) that pacman can be. 
    # Then since it can be only at exactly one we will use the function that we created in the previous question 
    ...
    # Then append the result of the sensorModel with arguments the items that pronuncation told us to use.
    ...
    # In the end return the pacphysics_sentences conjoined.
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # We are making the expression and append it to the knowledge base as I asked to.
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # I commented it because it was spamming my terminal with multiple prints while testing my code
        # print(t)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # Construct the model using the findModel and the goal state given the pronuncation
        ...
        # Construct the three sentences and then with the proper operators make the expressions and finally append them to the the knowledge base.
        for (x, y) in food:
...
def localization(problem, agent) -> Generator:
    ...
    # Create temporary lists and append them to the KB one for the walls that are in the list and one for the walls they aren't.
    ...
    for t in range(agent.num_timesteps):
        ...
        # I simply did what the pronuncation said to.
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add to KB and update known_map: (x, y) locations where there is provably a wall.
        # Add to KB and update known_map: (x, y) locations where there is provably not a wall.
        # Hint: check if the results of entails contradict each other (i.e. KB entails A and entails ¬ A). 
        # If they do, print feedback to help debugging.
        for (x, y) in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) == entails(conjoin(KB),
